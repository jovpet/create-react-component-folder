...
async function createReactMocks(componentName, pathToTypesDefinition) {
  ...
  return template({
    ...
    apolloLinkName: config.getValue('apollolink') ? config.getValue('apollolink').name : false,
    apolloLinkPath: config.getValue('apollolink') ? config.getValue('apollolink').path : false,
  });
}
 ...
 * @returns {String}
 */
async function createReactComponentStories(componentName, contextPath) {
  let patternName;
