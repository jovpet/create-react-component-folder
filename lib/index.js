...
const cli = program.version('1.1.0');
...
function createFiles(componentName, componentPath, cssFileExt) {
  return new Promise((resolve) => {
    ...
    const jsxExt = 'jsx';
    const indexFile = `index.${config.hasFlag('typescript') ? 'tsx' : 'js'}`;
    let name = componentName;
    const isJsxFile = config.hasFlag('jsx');
    const componentFileName = `${name}.view.${isJsxFile ? jsxExt : ext}`;
    ...
    // Add stateful pattern
    if (config.hasFlag('controller')) {
      files.push(`${name}.controller.${isJsxFile ? jsxExt : ext}`);
    }

    // add graphql pattern
    if (config.hasFlag('graphql')) {
      files.push(`${name}.controller.${isJsxFile ? jsxExt : ext}`);
      files.push(`${name}.apollo.${isJsxFile ? jsxExt : ext}`);
      files.push(`${name}.mocks.${isJsxFile ? jsxExt : ext}`);
    }
    ...
    fs.createDirectorys(componentPath)
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          } else if (file === `${name}.view.${ext}` || file === `${name}.view.${jsxExt}`) {
            const cssext = cssFileExt || 'css';
            promises.push(
              createReactComponent(componentName, cssext).then((template) =>
          ...
          } else if (file === `${name}.controller.${ext}` || file === `${name}.controller.${jsxExt}`) {
            promises.push(
              createReactController(componentName).then((template) =>
                // eslint-disable-next-line implicit-arrow-linebreak
                fs.writeFileAsync(
                  filePath,
                  formatPrettier(template, prettierParser, prettierOptions),
          ...
          } else if (file === `${name}.apollo.${ext}` || file === `${name}.apollo.${jsxExt}`) {
            const pathToTypesDefinition = path.relative(componentPath, path.join(ROOT_DIR, config.getValue('graphqldefs', '')));
            promises.push(
              createReactApollo(componentName, pathToTypesDefinition).then((template) =>
                // eslint-disable-next-line implicit-arrow-linebreak
                fs.writeFileAsync(
                  filePath,
                  formatPrettier(template, prettierParser, prettierOptions),
          ...
          } else if (file === `${name}.mocks.${ext}` || file === `${name}.mocks.${jsxExt}`) {
            const pathToTypesDefinition = path.relative(componentPath, path.join(ROOT_DIR, config.getValue('graphqldefs', '')));
            promises.push(
              createReactMocks(componentName, pathToTypesDefinition).then((template) =>
                // eslint-disable-next-line implicit-arrow-linebreak
                fs.writeFileAsync(
                  filePath,
                  formatPrettier(template, prettierParser, prettierOptions),
          ...
          if ((file.indexOf(`.stories.${ext}`) || file.indexOf(`.stories.${ext}`)) > -1) {
            let apolloLinkPath = '';
            const contextPath = path.join(ROOT_DIR, config.getValue('storiescontext'));
            const storyPath = path.relative(contextPath, componentPath);
            if (config.hasFlag('graphql') && config.getValue('apollolink')) {
              apolloLinkPath = path.relative(componentPath, config.getValue('apollolink').path);
            }
            promises.push(
              createReactComponentStories(name, storyPath, apolloLinkPath).then((template) =>
                // eslint-disable-next-line implicit-arrow-linebreak
                fs.writeFileAsync(
                  filePath,
                  formatPrettier(template, prettierParser, prettierOptions),
            ...
            fileExt = fileExt.indexOf('.', 0) > -1 ? fileExt.substr(1) : fileExt;
            const prettierStyleParser = fileExt !== 'ts' && fileExt !== 'js' ? fileExt : prettierParser;
            promises.push(
              createReactComponentStyle(name).then((template) =>
                // eslint-disable-next-line implicit-arrow-linebreak
                fs.writeFileAsync(
                  filePath,
                  formatPrettier(template, prettierStyleParser, prettierOptions),
                )),
            );
          }
        }
